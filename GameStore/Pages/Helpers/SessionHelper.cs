...
namespace GameStore.Pages.Helpers
{
	...
	public static class SessionHelper
	{
		...
		public static Profile GetProf(HttpSessionState session)
		{
			Profile myProfile = Get<Profile>(session, SessionKey.CART);
			if (myProfile == null)
			{
				myProfile = new Profile();
				Set(session, SessionKey.CART, myProfile);
			}
			return myProfile;
		}

		public static Cart GetInn(HttpSessionState session)
		{
			Cart myCart = Get<Cart>(session, SessionKey.CART);
			if (myCart == null)
			{
